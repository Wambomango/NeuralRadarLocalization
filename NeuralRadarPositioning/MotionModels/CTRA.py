...
class CTRA(MotionModel):
    def __init__(self, config): 
        ...
        self.trajectories = config["beacon"]["movement"]["motion_models"]["CTRA"]["trajectories"]
        self.minimum_initial_velocity = config["beacon"]["movement"]["motion_models"]["CTRA"]["minimum_initial_velocity"]
        self.maximum_initial_velocity = config["beacon"]["movement"]["motion_models"]["CTRA"]["maximum_initial_velocity"]
        self.minimum_acceleration = config["beacon"]["movement"]["motion_models"]["CTRA"]["minimum_acceleration"]
        self.maximum_acceleration = config["beacon"]["movement"]["motion_models"]["CTRA"]["maximum_acceleration"]
        self.minimum_turn_rate = config["beacon"]["movement"]["motion_models"]["CTRA"]["minimum_turn_rate"]
        self.maximum_turn_rate = config["beacon"]["movement"]["motion_models"]["CTRA"]["maximum_turn_rate"]

        self.SIMULATION_OVERSAMPLING = 100
    ...
    def generate_trajectories(self, n_trajectories = None):
        ...
        for i in tqdm(range(n_trajectories)):
            ...
            state_history[0,3] = np.random.uniform(self.minimum_initial_velocity, self.maximum_initial_velocity)
            state_history[0,4] = np.random.uniform(self.minimum_acceleration, self.maximum_acceleration)
            state_history[0,5] = np.random.uniform(self.minimum_turn_rate, self.maximum_turn_rate)
            ...
            for j in range(1, self.trajectory_steps):
                ...
                a_j = a_j_1
                omega_j = omega_j_1
                x_j = x_j_1 + (v_j * np.sin(phi_j) - v_j_1 * np.sin(phi_j_1)) / omega_j + a_j * (np.cos(phi_j) - np.cos(phi_j_1)) / omega_j_1**2
                y_j = y_j_1 - (v_j * np.cos(phi_j) - v_j_1 * np.cos(phi_j_1)) / omega_j + a_j * (np.sin(phi_j) - np.sin(phi_j_1)) / omega_j_1**2
            ...
            for j in range(self.samples_per_trajectory):
                flat_trajectory[j,0] = state_history[j * self.SIMULATION_OVERSAMPLING, 0]
                flat_trajectory[j,1] = state_history[j * self.SIMULATION_OVERSAMPLING, 1]
